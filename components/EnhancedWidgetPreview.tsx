...
const EnhancedWidgetPreview = ({ 
  ...
  const handleCarouselPrev = () => {
    setCarouselTranslateX(prev => Math.min(0, prev + CAROUSEL_SLIDE_WIDTH));
  };

  const handleCarouselNext = () => {
    const numSlides = filteredReviews.slice(0, 10).length;
    const maxOffset = -((numSlides - 1) * CAROUSEL_SLIDE_WIDTH); 
    setCarouselTranslateX(prev => Math.max(maxOffset, prev - CAROUSEL_SLIDE_WIDTH));
  };

  useEffect(() => {
    setCarouselTranslateX(0);
  }, [settings.layout, filteredReviews]);
    ...
    >
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center min-w-0">
          ...
          <div className="ml-3">
          <h4 className="font-semibold text-card-foreground dark:text-card-foreground transition-theme truncate" title={businessName}>{businessName}</h4>
            <div className="flex items-center">
              <Rating value={avgRating} size="lg"/>
              <span className="ml-2 text-sm font-medium text-card-muted-foreground dark:text-card-muted-foreground transition-theme">
        ...
        <div className="flex items-center flex-shrink-0">
          <span className="text-card-muted-foreground dark:text-card-muted-foreground mr-2 text-sm transition-theme text-right">
      ...
      {settings.layout === 'grid' && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
          {filteredReviews.slice(0, 6).map((review: IReviewItemFromAPI, index) => (
            <div key={getReviewKey(review, index)} className="bg-accent/50 dark:bg-accent/50 shadow-sm rounded-lg p-4 transition-theme border border-border/50 dark:border-border/50">
              <div className="flex items-center mb-3">
                {settings.showProfilePictures && (
                 ...
                 <div className={settings.showProfilePictures ? "ml-3" : ""}>
                  ...
                  {settings.showDates && (<span className="text-card-muted-foreground dark:text-card-muted-foreground text-xs transition-theme">{review.postedAt}</span>)}
                </div>
              </div>
              ...
              <p className="text-card-foreground dark:text-card-foreground text-sm leading-relaxed transition-theme line-clamp-4">{review.content}</p>
            </div>
          ))}
        </div>
      )}
        ...
        <div className="mb-4">
          <div className="relative w-full">
            <button
              ...
              className="absolute left-0 top-1/2 -translate-y-1/2 z-10 w-8 h-8 rounded-full bg-white dark:bg-gray-800 flex items-center justify-center text-gray-500 shadow-md border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 disabled:opacity-50"
            ...
            <div className="overflow-hidden px-8">
              ...
              >
                {filteredReviews.slice(0, 10).map((review: IReviewItemFromAPI, index) => (
                  <div 
            ...
            <button 
              ...
              className="absolute right-0 top-1/2 -translate-y-1/2 z-10 w-8 h-8 rounded-full bg-white dark:bg-gray-800 flex items-center justify-center text-gray-500 shadow-md border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 disabled:opacity-50"
              aria-label="Next review"
              disabled={carouselTranslateX <= -((filteredReviews.slice(0, 10).length - 1) * CAROUSEL_SLIDE_WIDTH)}
            >
        ...
        <div className="mb-4">
          <div className="columns-1 md:columns-2 lg:columns-3 gap-4 space-y-4">
            {filteredReviews.slice(0, 9).map((review, index) => {
              // const heightClass = index % 3 === 0 ? "pb-[120%]" : index % 3 === 1 ? "pb-[100%]" : "pb-[80%]";
              return (
                <div 
      ...
      <div className="mt-6 text-center text-card-muted-foreground dark:text-card-muted-foreground text-xs transition-theme">
